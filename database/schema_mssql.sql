-- MS SQL Server Schema for Tenant Analytics
...
-- Dimension: Account
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'dim_account')
...
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'dim_transaction_type')
...
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'dim_sales_hierarchy')
...
-- Fact: Transactions
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'fact_transactions')
CREATE TABLE fact_transactions (
    ...
    transaction_type_key INT FOREIGN KEY REFERENCES dim_transaction_type(transaction_type_key),
...
-- Fact: Loans
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'fact_loans')
CREATE TABLE fact_loans (
    loan_key BIGINT IDENTITY(1,1) PRIMARY KEY,
    date_key INT FOREIGN KEY REFERENCES dim_date(date_key),
    customer_key INT FOREIGN KEY REFERENCES dim_customer(customer_key),
...
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'fact_primary_sales')
CREATE TABLE fact_primary_sales (
    primary_sale_key BIGINT IDENTITY(1,1) PRIMARY KEY,
    date_key INT FOREIGN KEY REFERENCES dim_date(date_key),
    product_key INT FOREIGN KEY REFERENCES dim_product(product_key),
...
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'fact_account_balances')
...
-- Fact: Investments
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'fact_investments')
CREATE TABLE fact_investments (
    investment_key BIGINT IDENTITY(1,1) PRIMARY KEY,
    date_key INT FOREIGN KEY REFERENCES dim_date(date_key),
    customer_key INT FOREIGN KEY REFERENCES dim_customer(customer_key),
    product_key INT FOREIGN KEY REFERENCES dim_product(product_key),
